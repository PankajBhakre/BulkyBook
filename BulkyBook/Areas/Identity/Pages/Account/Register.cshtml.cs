        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if(!await _roleManager.RoleExistsAsync(StaticDetails.Role_Admin)) //If not present it will create Role
                    {
                        await _roleManager.CreateAsync(new IdentityRole(StaticDetails.Role_Admin));
                    }
                    if (!await _roleManager.RoleExistsAsync(StaticDetails.Role_Employee)) //If not present it will create Role
                    {
                        await _roleManager.CreateAsync(new IdentityRole(StaticDetails.Role_Employee));
                    }
                    if (!await _roleManager.RoleExistsAsync(StaticDetails.Role_User_Company)) //If not present it will create Role
                    {
                        await _roleManager.CreateAsync(new IdentityRole(StaticDetails.Role_User_Company));
                    }
                    if (!await _roleManager.RoleExistsAsync(StaticDetails.Role_User_Individual)) //If not present it will create Role
                    {
                   ...
                   if(user.Role == null)
                    ...
                    else
                    {
                        ...
                        {
                            // admin is registering the new user
